...
package.preload["fennel.binary"] = package.preload["fennel.binary"] or function(...)
  ...
  local _879_ = require("fennel.utils")
  local copy = _879_["copy"]
  local warn = _879_["warn"]
  ...
  local function execute(cmd)
    local _880_0 = os.execute(cmd)
    if (_880_0 == 0) then
      return true
    elseif (_880_0 == true) then
      return true
    end
  end
  ...
  local function module_name(open, rename, used_renames)
    ...
    do
      local _885_0 = rename[open]
      if (nil ~= _885_0) then
        local renamed = _885_0
  ...
  local function fennel__3ec(filename, native, options)
    ...
    local _889_
    do
      _889_ = "(do (local bundle_2_ ...) (fn loader_3_ [name_4_] (match (or (. bundle_2_ name_4_) (. bundle_2_ (.. name_4_ \".init\"))) (mod_5_ ? (= \"function\" (type mod_5_))) mod_5_ (mod_5_ ? (= \"string\" (type mod_5_))) (assert (if (= _VERSION \"Lua 5.1\") (loadstring mod_5_ name_4_) (load mod_5_ name_4_))) nil (values nil (: \"\n\\tmodule '%%s' not found in fennel bundle\" \"format\" name_4_)))) (table.insert (or package.loaders package.searchers) 2 loader_3_) ((assert (loader_3_ \"%s\")) ((or unpack table.unpack) arg)))"
    end
    fennel_loader = _889_:format(dotpath_noextension)
    local lua_loader = fennel["compile-string"](fennel_loader)
    local _890_ = options
    local rename_modules = _890_["rename-modules"]
  ...
  local function compile_binary(lua_c_path, executable_name, static_lua, lua_include_dir, native)
    ...
    local _892_
    do
      local _891_0 = shellout((cc .. " -dumpmachine"))
      if (nil ~= _891_0) then
        _892_ = _891_0:match("mingw")
      else
        _892_ = _891_0
      end
    end
    ...
    compile_command = {cc, "-Os", lua_c_path, table.concat(native, " "), static_lua, rdynamic, "-lm", _895_, "-o", (executable_name .. bin_extension), "-I", lua_include_dir, os.getenv("CC_OPTS")}
  ...
  local function native_path_3f(path)
    ...
    if (version_extension and (version_extension ~= "") and not version_extension:match("%.%d+")) then
      ...
      local _900_0 = extension
      if (_900_0 == "a") then
        return path
      elseif (_900_0 == "o") then
        return path
      elseif (_900_0 == "so") then
        return path
      elseif (_900_0 == "dylib") then
        return path
      else
        local _ = _900_0
        return false
  ...
  local function compile(filename, executable_name, static_lua, lua_include_dir, options, args)
    local _907_ = extract_native_args(args)
    local libraries = _907_["libraries"]
    local modules = _907_["modules"]
    local rename_modules = _907_["rename-modules"]
...
local function handle_lua(i)
  ...
  local _854_0, _855_0 = os.execute(table.concat(cmd, " "))
  if (((_854_0 == true) and (_855_0 == "exit")) or (_854_0 == 0)) then
...
do
  ...
  while (arg[i] and not options["ignore-options"]) do
    ...
    elseif (_860_0 == "--skip-include") then
      ...
      table.remove(arg, i)
    elseif (_860_0 == "--use-bit-lib") then
...
local function eval(form)
  ...
  return print(dosafely(fennel.eval, _870_, options))
end
local function compile(files)
  for _, filename in ipairs(files) do
    ...
    do
      local _873_0, _874_0 = nil, nil
      local function _875_()
        return fennel["compile-string"](f:read("*a"), options)
      end
      _873_0, _874_0 = xpcall(_875_, fennel.traceback)
      if ((_873_0 == true) and (nil ~= _874_0)) then
        local val = _874_0
        print(val)
      elseif (true and (nil ~= _874_0)) then
        local _0 = _873_0
        local msg = _874_0
...
if ((_G.type(_877_0) == "table") and _878_(...)) then
  return repl()
elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "--repl")) then
  return repl()
elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "--compile")) then
  local files = {select(2, (table.unpack or _G.unpack)(_877_0))}
  return compile(files)
elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "-c")) then
  local files = {select(2, (table.unpack or _G.unpack)(_877_0))}
  return compile(files)
elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "--compile-binary") and (nil ~= _877_0[2]) and (nil ~= _877_0[3]) and (nil ~= _877_0[4]) and (nil ~= _877_0[5])) then
  local filename = _877_0[2]
  local out = _877_0[3]
  local static_lua = _877_0[4]
  local lua_include_dir = _877_0[5]
  local args = {select(6, (table.unpack or _G.unpack)(_877_0))}
  local bin = require("fennel.binary")
  options.filename = filename
  options.requireAsInclude = true
  return bin.compile(filename, out, static_lua, lua_include_dir, options, args)
elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "--compile-binary")) then
  local cmd = (arg[0] or "fennel")
  return print((require("fennel.binary").help):format(cmd, cmd, cmd))
elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "--eval") and (nil ~= _877_0[2])) then
  local form = _877_0[2]
  return eval(form)
elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "-e") and (nil ~= _877_0[2])) then
  local form = _877_0[2]
  return eval(form)
else
  ...
  if (((_G.type(_877_0) == "table") and (nil ~= _877_0[1])) and _908_(...)) then
    local a = _877_0[1]
    return print(fennel["runtime-version"]())
  elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "--help")) then
    return print(help)
  elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "-h")) then
    return print(help)
  elseif ((_G.type(_877_0) == "table") and (_877_0[1] == "-")) then
    return dosafely(fennel.eval, (io.stdin):read("*a"))
  elseif ((_G.type(_877_0) == "table") and (nil ~= _877_0[1])) then
    local filename = _877_0[1]
    local args = {select(2, (table.unpack or _G.unpack)(_877_0))}
    arg[-2] = arg[-1]
    arg[-1] = arg[0]
